...
<c:import url="/mobile/layout/sub-header"/>
    ...
    <input type="hidden" name="product_delivery_International_type" value="A"/>
        ...
        <ul class="taps">
            <li><a href="/MyPage/DeliveryAddress" id="tap1" class="active">${afn:getMessage('domestic_delivery_addr',sessionScope.locale)}</a></li>
            <li><a href="/MyPage/DeliveryAddressForeign" id="tap2">${afn:getMessage('foreign_delivery_addr',sessionScope.locale)}</a></li>
        </ul>
    </section>
    <section class="wrap clearfix" id="write">
            ...
            <thead class="lis-head">
                <tr>
                    <th colspan="2" class="c-btn">
                        <button type="button" id="updateDeliveryInfoM">${afn:getMessage('check_addr',sessionScope.locale)} ${afn:getMessage('update',sessionScope.locale)}</button>
        ...
        <div class="fot-btn">
            <button type="button" onclick="location.href='/MyPage/DeliveryAddressInsert'">${afn:getMessage('new_addr',sessionScope.locale)}</button>
...
$("#updateDeliveryInfoM").click(function(){
	...
	if(!formData.includes("orderNo=")){
		alert("${afn:getMessage('error.check_addr',sessionScope.locale)}");
		return;
	}
